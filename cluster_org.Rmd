...
```{r}
CHAPALA_previo<- read.csv("org.csv", header=TRUE, stringsAsFactors=FALSE)
...
CHAPALA_NA <- sapply(CHAPALA_previo, function(x) sum(is.na(x))/CH_n)[4:5]
...
nom_E <- dist_CH_p$EST
dist_CH <- dist_CH_p %>%  dplyr::select(COT:COT_SOL)  %>% scale(center=TRUE, scale = TRUE)%>% as.data.frame()
