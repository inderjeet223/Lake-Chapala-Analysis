...
```{r}
CHAPALA_previo<- read.csv("physical.csv", header=TRUE, stringsAsFactors=FALSE)
...
CHAPALA_NA <- sapply(CHAPALA_previo, function(x) sum(is.na(x))/CH_n)[4:29]
...
load("LDA_physical.RData")
```

```{r}
dist_CH_p <- CHAPALA %>% group_by(EST) %>%  dplyr::summarise_if(is.numeric, mean)
nom_E <- dist_CH_p$EST
dist_CH <- dist_CH_p %>%  dplyr::select(COLOR_VER:TEMP_AGUA_2)  %>% scale(center=TRUE, scale = TRUE)%>% as.data.frame()
