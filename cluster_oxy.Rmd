...
```{r}
CHAPALA_previo<- read.csv("oxy.csv", header=TRUE, stringsAsFactors=FALSE)
...
CHAPALA_NA <- sapply(CHAPALA_previo, function(x) sum(is.na(x))/CH_n)[4:13]
CHAPALA_M <- names(CHAPALA_NA[CHAPALA_NA<0.9])
...
nom_E <- dist_CH_p$EST
dist_CH <- dist_CH_p %>%  dplyr::select(OD_._SUP:POT_REDOX_CAMPO_SUP)  %>% scale(center=TRUE, scale = TRUE)%>% as.data.frame()
