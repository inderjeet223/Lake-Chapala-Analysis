...
```{r}
CHAPALA_previo<- read.csv("micro.csv", header=TRUE, stringsAsFactors=FALSE)
...
CHAPALA_NA <- sapply(CHAPALA_previo, function(x) sum(is.na(x))/CH_n)[4:6]
...
load("LDA_micro.RData")
```

```{r}
dist_CH_p <- CHAPALA %>% group_by(EST) %>%  dplyr::summarise_if(is.numeric, mean)
nom_E <- dist_CH_p$EST
dist_CH <- dist_CH_p %>%  dplyr::select(COLI_FEC:E_COLI)  %>% scale(center=TRUE, scale = TRUE)%>% as.data.frame()
