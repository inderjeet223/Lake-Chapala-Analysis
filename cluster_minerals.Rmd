...
```{r}
CHAPALA_previo<- read.csv("minerals.csv", header=TRUE, stringsAsFactors=FALSE)
...
CHAPALA_NA <- sapply(CHAPALA_previo, function(x) sum(is.na(x))/CH_n)[4:7]
...
load("LDA_minerals.RData")
```

```{r}
dist_CH_p <- CHAPALA %>% group_by(EST) %>%  dplyr::summarise_if(is.numeric, mean)
nom_E <- dist_CH_p$EST
dist_CH <- dist_CH_p %>%  dplyr::select(DBO_SOL:DQO_TOT)  %>% scale(center=TRUE, scale = TRUE)%>% as.data.frame()
